...
public class CreateLineByTCore{
    ...
    MNK mnk = new MNK();
    static double[][] mnkArray;
    static double[] coefficientMNKToArray;
    static double errorTArray;
    static double correlationIndex;
...
//                sigmaT=kx+sigma0 - уравнение прямой через угловой к-ф
            ...
            double[][] mnkArray = createMNKArray(res, i, arrayValue);
            double[] coefficientMNKtoTArray = mnk.method(mnkArray);
            double errorTArray = mnk.checkError(mnkArray, coefficientMNKtoTArray);
            double correlationIndex = correlation(mnkArray);
            if(errorTArray<error && correlationIndex>correlationValue){
                error = errorTArray;
                correlationValue = correlationIndex;
                result[0] = 1;
                result[1] = i;
...
//                sigmaT=kx+sigma0 - уравнение прямой через угловой к-ф
    ...
    public double[] findT2Core(double[][] arrayValue) {
        ...
        t2ValueY02[0]=new double[]{2.577,4,493,7.811,10.78,13.53,16.13,18.62,25.59,38.09,65,88.58,110.1,149.3,185,249.3,307.4,361,411.2,667.7,1063,2471,3310};
        t2ValueY02[1]=new double[]{1.601,2.757,4.721,6.447,8.031,9.514,10.92,14.8,21.61,35.74,47.66,38.25,76.86,93.16,121.3,145.3,166.8,186,274.3,382.8,493.3,567.1};
        t2ValueY02[2]=new double[]{1.127,1.913,3.221,4.349,5.368,6.31,7.195,9.596,13.68,21.74,28.18,33.65,42.77,50.27,62.2,71.46,78.93,85.09,106.4,117.8};
        t2ValueY02[3]=new double[]{0.86,1.431,2.353,3.13,3.819,4.448,5.029,6.574,9.097,13.74,17.14,19.83,23.91,26.8,30.8,33.16,34.57,35.39};
        t2ValueY02[4]=new double[]{0.66,1.081,1.744,2.285,2.754,3.173,3.555,4.536,6.047,8.53,10.10,11.17,12.47,13.13,13.56};
        t2ValueY02[5]=new double[]{0.521,0.837,1.316,1.694,2.011,2.286,2.531,3.133,3.979,5.124,5.647,5.885,6.003};

        double [][] res = new double[arrayValue.length][t2ValueY01.length];
        for (int i = 0; i < t1ValueK01.length; i++) {
//                sigmaT=kx+sigma0 - уравнение прямой через угловой к-ф
            double k = (arrayValue[0][1] - arrayValue[arrayValue.length - 1][1]) / (t1ValueK01[t1ValueK01.length-1][i] - t1ValueK01[0][i]);
            double sigma0 = (arrayValue[0][1] + k * t1ValueK01[0][i]);
            for (int j = 0; j < arrayValue.length; j++) {

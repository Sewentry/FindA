...
//                sigmaT=kx+sigma0 - уравнение прямой через угловой к-ф
            ...
            if(errorTArray<errorT1 && correlationIndex>correlationValueT1){
                ...
                result[3] = sigma0;
                result[4] = result[3]/k*Math.pow(10,6)/t1ValueK01[t1ValueK01.length-1][i];
...
//                sigmaT=kx+sigma0 - уравнение прямой через угловой к-ф
            ...
            double correlationIndex = correlation(mnkArrayToCorrelation);
                if(errorTArray<errorT1 && correlationIndex>correlationValueT1){
                    ...
                    result[2]=correlationIndex;
                    result[3]=sigma0;
                    result[4] = result[3]/k*Math.pow(10,6)/t1ValueK001[t1ValueK001.length-1][i];
                }
        }
...
//                sigmaT=kx+sigma0 - уравнение прямой через угловой к-ф
            ...
            if(errorTArray<errorT1 && correlationIndex>correlationValueT1){
                ...
                result[2]=correlationIndex;
                result[3]=sigma0;
                result[4] = result[3]/k*Math.pow(10,6)/t1ValueK0001[t1ValueK0001.length-1][i];
            }
        }
        for (int i = 0; i < t1ValueKO0001[0].length; i++) {
//                sigmaT=kx+sigma0 - уравнение прямой через угловой к-ф
            double k = (arrayValue[0][1] - arrayValue[arrayValue.length - 1][1]) / (t1ValueKO0001[t1ValueKO0001.length-1][i] - t1ValueKO0001[0][i]);
            double sigma0 = (arrayValue[0][1] + k * t1ValueKO0001[0][i]);
            for (int j = 0; j < arrayValue.length; j++) {
                res[j][i]=sigma0 - k * t1ValueKO0001[j][i];
                //sumOptimalLine += Math.pow(arrayValue[j][1] - (optimizeLine[1] + optimizeLine[0] * arrayValue[j][i]), 2);
            }
            double[][] mnkArray = createMNKArray(res, i, t1ValueKO0001);
            double[][] mnkArrayToCorrelation=createMNKArrayExperiment(res,i,arrayValue);
            double[] coefficientMNKtoTArray = mnk.method(mnkArray);
            double errorTArray = mnk.checkError(mnkArray, coefficientMNKtoTArray);
            double correlationIndex = correlation(mnkArrayToCorrelation);
        ...
        for (int i = 0; i < t1ValueK000001[0].length; i++) {
//                sigmaT=kx+sigma0 - уравнение прямой через угловой к-ф
            double k = (arrayValue[0][1] - arrayValue[arrayValue.length - 1][1]) / (t1ValueK000001[t1ValueK000001.length-1][i] - t1ValueK000001[0][i]);
            double sigma0 = (arrayValue[0][1] + k * t1ValueK000001[0][i]);
            for (int j = 0; j < arrayValue.length; j++) {
                res[j][i]=sigma0 - k * t1ValueK000001[j][i];
                //sumOptimalLine += Math.pow(arrayValue[j][1] - (optimizeLine[1] + optimizeLine[0] * arrayValue[j][i]), 2);
            }
            double[][] mnkArray = createMNKArray(res, i, t1ValueK000001);
            double[][] mnkArrayToCorrelation=createMNKArrayExperiment(res,i,arrayValue);
            double[] coefficientMNKtoTArray = mnk.method(mnkArray);
            double errorTArray = mnk.checkError(mnkArray, coefficientMNKtoTArray);
            double correlationIndex = correlation(mnkArrayToCorrelation);
            if(errorTArray<errorT1 && correlationIndex>correlationValueT1){
                ...
                result[2]=correlationIndex;
                result[3]=sigma0;
                result[4] = result[3]/k*Math.pow(10,6)/t1ValueK000001[t1ValueK000001.length-1][i];
            }
        }
        return result;
    }
    public double[] findT2Core(double[][] arrayValue) {
        ...
        t2ValueY04[0]=new double[]{2.456,3.712,5.526,6.909,8.053,9.036,9.902,12.02,14.97,18.99,20.87,21.750,22.21};
        t2ValueY04[1]=new double[]{3.007,4.583,6.904,8.71,10.23,11.56,12.74,15.720,20.11,26.900,30.87,33.38,36.05,37.06};
        t2ValueY04[2]=new double[]{3.858,5.928,9.03,11.49,13.59,15.45,17.14,21.45,28.12,39.41,46.97,52.53,60.11,64.86,69.76,71.49,71.86};
        t2ValueY04[3]=new double[]{5.37,8.31,12.79,16.4,19.54,22.34,24.91,31.62,42.36,61.83,76.13,87.58,105.3,118.7,137.6,150.3,158.9,164.9,175};
        t2ValueY04[4]=new double[]{8.928,13.89,21.560,27.85,33.37,38.36,42.97,55.22,75.44,114.1,114.4,170.1,212.9,248.4,305.9,352,390.5,423.4,553.9,664.6,706.5};
        t2ValueY04[5]=new double[]{29.87,46.81,73.33,95.35,114.8,132.7,149.3,193.9,269.5,420.5,544.7,654.1,845.6,1014,1306,1562,1792,2004,3047,4571,6734,9665,12595};

        t2ValueY05[0]=new double[]{4.738,6.85,9.792,11.98,13.76,15.28,16.6,19.81,24.23,30.18,32.95,34.25,34.92};
        t2ValueY05[1]=new double[]{5.767,8.384,12.08,14.87,17.170,19.16,20.91,25.25,31.51,40.87,46.140,49.37,52.58,53.6};
        t2ValueY05[2]=new double[]{7.42,10.83,15.710,19.45,22.57,25.29,27.74,33.85,43.03,57.93,67.48,74.22,82.96,87.97,92.33,93.24};
        t2ValueY05[3]=new double[]{10.42,15.28,22.33,27.8,32.42,36.50,40.17,49.57,64.13,89.34,107,120.7,141.1,155.6,174.8,186.2,193,196.9};
        t2ValueY05[4]=new double[]{17.95,26.42,38.84,28.6,56.93,64.34,71.08,88.57,116.4,167.3,205.5,236.9,287.6,328.1,391.2,439.5,478,509.7,620.3,680.2};
        t2ValueY05[5]=new double[]{102.4,150.4,221.1,277.2,325.4,368.6,408.1,511.9,681,1003,1258,1476,1850,2169,2713,3178,3590,3965,5757,8281,11745,16307,20775,34975};
...
//                sigmaT=kx+sigma0 - уравнение прямой через угловой к-ф
            ...
            for (int j = 0; j < Math.min(t2ValueY01.length,arrayValue.length); j++) {
            ...
            if(errorTArray<errorT2 && correlationIndex>correlationValueT2){
                ...
                result[2] = correlationIndex;
                result[3]=sigma0;
                result[4] = result[3]/k*Math.pow(10,6)/t2ValueY01[i][t2ValueY01[i].length-1];
            }
        }
...
//                sigmaT=kx+sigma0 - уравнение прямой через угловой к-ф
            ...
            for (int j = 0; j < Math.min(t2ValueY02.length,arrayValue.length); j++) {
            ...
            if(errorTArray<errorT2 && correlationIndex>correlationValueT2){
                ...
                result[2] = correlationIndex;
                result[3] = sigma0;
                result[4] = result[3]/k*Math.pow(10,6)/t2ValueY02[i][t2ValueY02[i].length-1];
            }
        }
...
//                sigmaT=kx+sigma0 - уравнение прямой через угловой к-ф
            ...
            for (int j = 0; j < Math.min(t2ValueY03.length,arrayValue.length); j++) {
            ...
            if(errorTArray<errorT2 && correlationIndex>correlationValueT2){
                ...
                result[2] = correlationIndex;
                result[3] = sigma0;
                result[4] = result[3]/k*Math.pow(10,6)/t2ValueY03[i][t2ValueY03[i].length-1];
            }
        }
        for (int i = 0; i < t2ValueY04.length; i++) {
//                sigmaT=kx+sigma0 - уравнение прямой через угловой к-ф
            double k = (arrayValue[0][1] - arrayValue[t2ValueY04.length - 1][1]) / (t2ValueY04[i][t2ValueY04[0].length-1] - t2ValueY04[0][t2ValueY04[0].length-1]);
            double sigma0 = (arrayValue[0][1] + k * t2ValueY04[i][0]);
            for (int j = 0; j < Math.min(t2ValueY04.length,arrayValue.length); j++) {
                res[j][i] = sigma0 - k * t2ValueY04[i][j];
                //sumOptimalLine += Math.pow(arrayValue[j][1] - (optimizeLine[1] + optimizeLine[0] * arrayValue[j][i]), 2);
            }
            double[][] mnkArray = createMNKArray(res, i, t2ValueY04);
            double[][] mnkArrayToCorrelation=createMNKArrayExperiment(res,i,arrayValue);
            double[] coefficientMNKtoTArray = mnk.method(mnkArray);
            double errorTArray = mnk.checkError(mnkArray, coefficientMNKtoTArray);
            double correlationIndex = correlation(mnkArrayToCorrelation);
            if(errorTArray<errorT2 && correlationIndex>correlationValueT2){
                ...
                result[2] = correlationIndex;
                result[3] = sigma0;
                result[4] = result[3]/k*Math.pow(10,6)/t2ValueY04[i][t2ValueY04[i].length-1];
            }
        }
        for (int i = 0; i < t2ValueY05.length; i++) {
//                sigmaT=kx+sigma0 - уравнение прямой через угловой к-ф
            double k = (arrayValue[0][1] - arrayValue[t2ValueY05.length - 1][1]) / (t2ValueY05[i][t2ValueY05[0].length-1] - t2ValueY05[0][t2ValueY05[0].length-1]);
            double sigma0 = (arrayValue[0][1] + k * t2ValueY05[i][0]);
            for (int j = 0; j < Math.min(t2ValueY05.length,arrayValue.length); j++) {
                res[j][i] = sigma0 - k * t2ValueY05[i][j];
                //sumOptimalLine += Math.pow(arrayValue[j][1] - (optimizeLine[1] + optimizeLine[0] * arrayValue[j][i]), 2);
            }
            double[][] mnkArray = createMNKArray(res, i, t2ValueY05);
            double[][] mnkArrayToCorrelation=createMNKArrayExperiment(res,i,arrayValue);
            double[] coefficientMNKtoTArray = mnk.method(mnkArray);
            double errorTArray = mnk.checkError(mnkArray, coefficientMNKtoTArray);
            double correlationIndex = correlation(mnkArrayToCorrelation);
            if(errorTArray<errorT2 && correlationIndex>correlationValueT2){
                ...
                result[2] = correlationIndex;
                result[3] = sigma0;
                result[4] = result[3]/k*Math.pow(10,6)/t2ValueY05[i][t2ValueY05[i].length-1];
            }
        }
        return result;
    }
